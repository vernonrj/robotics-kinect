Public Class Form1
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click, TextBox1.TextChanged
    ...
    Private Sub TextBox2_TextChanged(sender As Object, e As EventArgs) Handles TextBox2.TextChanged
        ...
        Dim i As Integer
        Try
            byteOut(0) = Len(TextBox2.Text) + 5  'number bytes in output message
            byteOut(1) = &H0  'should be 0 for NXT
            byteOut(2) = &H80 '&H0 = reply expected &H80 = no reply expected
            byteOut(3) = &H9  'Send Bluetooth
            byteOut(4) = &H0  'Box Number - 1
            byteOut(5) = Len(TextBox2.Text) + 1  'message size with null terminator
            For i = 1 To Len(TextBox2.Text) 'copy bytes into output array
                byteOut(i + 5) = Asc(Mid(TextBox2.Text, i, 1))
            Next
            byteOut(Len(TextBox2.Text) + 6) = &H0 'add null terminator
            SerialPort1.Write(byteOut, 0, Len(TextBox2.Text) + 7) 'send message
            TextBox2.Clear()
        Catch ex As Exception
            MsgBox(ex.ToString)
